...
const CreateNoteModal = ({ isOpen, onClose, onSubmit, categories }: any) => {
  const [form, setForm] = useState({ title: '', description: '', categoryId: '', durationMinutes: 60, tags: '' });
      ...
      <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-8 overflow-y-auto space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Duration (min)</label>
              <input
                type="number"
        ...
        <div className="p-6 border-t border-gray-100 bg-gray-50/50 flex justify-end gap-3">
          <button onClick={onClose} className="px-6 py-2.5 rounded-xl text-gray-500 font-medium hover:bg-gray-100 transition">Cancel</button>
          <button
            onClick={() => onSubmit({ ...form, id: Date.now().toString(), date: new Date().toISOString(), tags: form.tags.split(',').map(t => t.trim()).filter(Boolean) })}
