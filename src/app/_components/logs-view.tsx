...
// --- Logs View ---
const LogsView = ({ logs, categories, onDeleteLog, onAddLog }: any) => (
  <div className="space-y-6">
    <div className="flex justify-between items-center">
      <h2 className="text-3xl font-bold text-gray-800">Learning Logs</h2>
      <Button onClick={onAddLog}><Plus size={20} /> New Log</Button>
    </div>
    <div className="grid gap-4">
      {logs.length === 0 ? <EmptyState icon={FileText} message="Start logging your journey!" /> : logs.map((log: any) => {
          ...
          <Card key={log.id} className="hover:border-purple-200 transition group">
            <div className="flex justify-between items-start">
              ...
              <div className="flex flex-col items-end gap-2">
                ...
                <button onClick={() => { }} className="text-xs text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition">Delete</button>
              </div>
            </div>
          </Card>
        );
